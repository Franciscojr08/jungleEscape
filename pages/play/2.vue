<template>
  <div class="game">
    ...
    <div class="resposta" @click="abreResposta1()">
      Foge para bem longe
    </div>
    <div class="resposta" @click="abreResposta2()">
      Pega o bilhete e foge para bem longe
    </div>
    <div class="resposta" @click="abreResposta3()">
      Puxa toda coragem do coração e enfrenta o homem
    </div>
    <div id="resposta1" class="resposta_e">
      <p>
        Você consegue fugir do local e voltar à cidade, porém sem nenhuma ideia
      ...
      <button class="btn_play_game">
        <nuxt-link to="/" class="link_game">
          Início do Jogo
        </nuxt-link>
      </button>
    </div>
    <div id="resposta2" class="resposta_e">
      <p>
        O homem lhe alcançou enquanto você tentava pegar o bilhete e lhe acertou
    ...
    <div id="resposta3" class="resposta_v">
      <p>
        Você consegue desviar da primeira marretada do homem, que fica ocioso
        por um curto período, o suficiente para você acertá-lo em cheio na cabeça e
        apagá-lo no chão.
      </p>
      <button class="btn_play_game">
        <nuxt-link to="/play/3" class="link_game">
          Próxima fase
        </nuxt-link>
      </button>
    </div>
...
export default {
  data () {
    return {
      ...
      contagem3: 0,
      abreResposta1: () => {
        if (this.contagem2 === 0 && this.contagem3 === 0) {
          document.getElementById('resposta1').style.display = 'block'
          this.contagem1++
        } else if (this.contagem1 >= 1) {
          alert('Volte para o início do jogo!'); location.href = '../'
        } else {
      ...
      abreResposta2: () => {
        if (this.contagem1 === 0 && this.contagem3 === 0) {
          document.getElementById('resposta2').style.display = 'block'
          this.contagem2++
        } else if (this.contagem2 >= 1) {
          alert('Volte para o início do jogo!'); location.href = '../'
        } else {
      ...
      abreResposta3: () => {
        if (this.contagem1 === 0 && this.contagem2 === 0) {
          document.getElementById('resposta3').style.display = 'block'
          this.contagem3++
        } else if (this.contagem3 >= 1) {
          alert('Volte para o início do jogo!'); location.href = '../'
        } else {
...
body {
  background: url(../../static/fundoJungle.png) 50%/cover no-repeat;
}
...
.btn_play_game:hover, .link_game:hover {
  cursor: pointer;
  color: #fff;
